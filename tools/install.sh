...
detect_gnss() {
    ...
    # detection on uart port
      if [[ ${#detected_gnss[*]} -ne 2 ]]; then
        ...
        for port in ttyUSB0 ttyS0 serial0 ttyUSB1 ttyUSB2 ttyS1 ttyS2 ttyS3; do
            for port_speed in 115200 57600 38400 19200 9600; do
                ...
                if [[ $(python3 "${rtkbase_path}"/tools/ubxtool -f /dev/$port -s $port_speed -p MON-VER -w 5 2>/dev/null) =~ 'ZED-F9P' ]]; then
                    ...
                    break
                elif [[ $(python3 "${rtkbase_path}"/tools/unicore_tool.py --port /dev/$port --baudrate $port_speed --command get_model --retry 2 2>/dev/null) =~ 'UM980 || UM982' ]]; then
                    detected_gnss[0]=$port
...
configure_gnss(){
    ...
    echo '################################'
      ...
      then
        ...
        elif [[ $(python3 "${rtkbase_path}"/tools/unicore_tool.py --port /dev/${com_port} --baudrate ${com_port_settings%%:*} --command get_model --retry 2 2>/dev/null) =~ 'UM980 || UM982' ]]
        then
